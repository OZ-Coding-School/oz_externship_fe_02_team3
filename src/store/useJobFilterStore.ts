...
function hasNumberId(j: JobPost): j is JobPost & { id: number } {
  return typeof j.id === 'number'
}
      ...
      applyFilters: (jobs: JobPost[]) => {
        ...
        if (selectedSort === '최신순') {
          filtered = [...filtered].sort((a, b) => {
            const A = hasNumberId(a) ? a.id : Number.MIN_SAFE_INTEGER
            const B = hasNumberId(b) ? b.id : Number.MIN_SAFE_INTEGER
            return B - A
          })
        } else if (selectedSort === '오래된순') {
          filtered = [...filtered].sort((a, b) => {
            const A = hasNumberId(a) ? a.id : Number.MAX_SAFE_INTEGER
            const B = hasNumberId(b) ? b.id : Number.MAX_SAFE_INTEGER
            return A - B
          })
